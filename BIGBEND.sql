USE [HIS_USER]
GO
  /****** Object:  UserDefinedFunction [GADS].[BIGBEND_MONTHLY_GEN_FN]    Script Date: 3/17/2025 8:06:16 AM ******/
SET
  ANSI_NULLS ON
GO
SET
...
GO
  CREATE FUNCTION [GADS].[BIGBEND_MONTHLY_GEN_FN] (@SearchUID VARCHAR(256), @StartDate DATETIME, @EndDate DATETIME) RETURNS @RESULTS TABLE(
    DATEPART INT,
    STATION VARCHAR(256),
    PLANT VARCHAR(256),
    UNIT VARCHAR(256),
...
,
  UNIT_GROSS AS (
    ...
    FROM
      PLANT_METER_READINGS -- UN#1, UN#2, UN#3 has been excluded as they are not included in gross generation calculation
    WHERE
      VALUETYPE = 'g'
    GROUP BY
      DATEPART,
